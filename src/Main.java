...
import lending_items.LendingQueue;
...
public class Main {
	...
	public static LendingQueue lendingItemList;
	...
	public static void main(String[] args) {
		...
		lendingItemList = new LendingQueue();
		lendingItemList.enqueue(new Lending("B03", "R05", 0));
		lendingItemList.enqueue(new Lending("B03", "R01", 1));
		lendingItemList.enqueue(new Lending("B04", "R02", 2));
		lendingItemList.enqueue(new Lending("B07", "R03", 1));
					...
					while (true) {
						...
						try {
							...
							if (result1 == 1) {
								...
								System.out
										.println("--Input & add to beginning--");
								// get book info from user's input
								getBookInfo();
	...
	public static void getLendingInfo() {
		// get bcode, bcode must not be empty and in book list
		while (true) {
			sc.nextLine();
			System.out.println("BCODE: ");
			bcode = sc.next();
			if (bcode.length() != 0) {
				...
				if (book != null) {
					...
					System.out.println("This book with Bcode: " + bcode
							+ " does not exist!");
				}
			} else {
				System.out.println("BCODE must not be empty!");
		...
		// get rcode, rcode must not be empty and in reader list
		while (true) {
			sc.nextLine();
			System.out.println("RCODE: ");
			rcode = sc.next();
			if (rcode.length() != 0) {
				...
				if (reader != null) {
					...
					System.out.println("This reader with Rcode: " + rcode
							+ " does not exist!");
				}
			} else {
				System.out.println("RCODE must not be empty!");
		...
		while (true) {
			...
			System.out.println("0. The book hasn't been given to reader");
			System.out
					.println("1. The book hasn't been returned by the reader");
			System.out.println("2. The book has been returned");
			try {
				state = sc.nextInt();
